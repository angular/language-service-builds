    ...
    if (type.diDeps) {
        ...
        try {
            for (var _iterator = type.diDeps[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                ...
                if (diDep.token.identifier && identifierName(diDep.token.identifier) == 'TemplateRef') return true;
            }
        } catch (err) {
            ...
            if (this.attr && getSelectors(this.info) && this.attr.name.startsWith(TEMPLATE_ATTR_PREFIX)) {
                ...
                if (!binding || binding.key == key && !binding.expression) {
                    ...
                    if (binding.expression && inSpan(valueRelativePosition, binding.expression.ast.span) || binding.key && valueRelativePosition > binding.span.start + (binding.key.length - key.length) || !binding.key) {
                        ...
                        this.attributeValueCompletions(binding.expression ? binding.expression.ast : new PropertyRead(span, new ImplicitReceiver(span), ''), valueRelativePosition);
                    } else {
                        keyCompletions();
                    }
                }
    ...
    try {
        for (var _iterator6 = element.attrs[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
            ...
            if (!attr.name.match(templateAttr)) {
                var _splitNsName = splitNsName(attr.name),
...
var ReflectorHost = function (_CompilerHost) {
    ...
    function ReflectorHost(getProgram, serviceHost, options) {
        ...
        var _this20 = _possibleConstructorReturn(this, (ReflectorHost.__proto__ || Object.getPrototypeOf(ReflectorHost)).call(this, null, options, new ModuleResolutionHostAdapter(new ReflectorModuleModuleResolutionHost(serviceHost))));
    ...
    _createClass(TypeScriptServiceHost, [{
        ...
        value: function ensureAnalyzedModules() {
            ...
            if (!analyzedModules) {
                ...
                var programSymbols = extractProgramSymbols(this.staticSymbolResolver, this.program.getSourceFiles().map(function (sf) {
                    return sf.fileName;
                }), analyzeHost);
                analyzedModules = this.analyzedModules = analyzeNgModules(programSymbols, analyzeHost, this.resolver);
            }
        ...
        get: function get() {
            ...
            if (!result) {
                ...
                result = this._resolver = new CompileMetadataResolver(config, moduleResolver, directiveResolver, pipeResolver, new SummaryResolver(), elementSchemaRegistry, directiveNormalizer, this._staticSymbolCache, this.reflector, function (error, type) {
        ...
        get: function get() {
            ...
            if (!result) {
                ...
                result = this._staticSymbolResolver = new StaticSymbolResolver(this.reflectorHost, this._staticSymbolCache, summaryResolver, function (e, filePath) {
        ...
        get: function get() {
            ...
            if (!result) {
                result = this._reflector = new StaticReflector(this.staticSymbolResolver, [], [], function (e, filePath) {
